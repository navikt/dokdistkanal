package no.nav.dokkanalvalg.config.cxf;

//import no.nav.dokkanalvalg.config.sts.STSConfig;
...
public abstract class AbstractCxfEndpointConfig {
	...
	protected <T> T createPort(Class<T> portType) {
		factoryBean.getFeatures().add(new no.nav.dokkanalvalg.config.cxf.TimeoutFeature(receiveTimeout, connectTimeout));
