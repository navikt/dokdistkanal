...
public class BestemDistribusjonskanalErrorHandler extends ResponseEntityExceptionHandler {
	...
	private static final Set<HttpStatus> HTTPSTATUS_ERROR_CODES = Set.of(NOT_FOUND, BAD_REQUEST, UNAUTHORIZED, FORBIDDEN);
	...
	ResponseEntity<Object> handleConsumerFunctionalException(Exception ex) {

		HttpStatus httpStatus = HTTPSTATUS_ERROR_CODES.stream()
				.filter(status -> ex.getMessage().contains(status.getReasonPhrase()))
				.findAny().orElse(null);
		...
		return ResponseEntity.status(HttpStatusCode.valueOf(httpStatus.value()))
